...
const lineItemFromOrder = (order: Order, materialName?: string) => {
    const unitPrice = Math.max(0, Math.round(order.harga_total / Math.max(1, order.quantity)));
    const baseName = (materialName?.trim() || order.kode_bahan).toUpperCase();
...
export const downloadOrderNotaPdf = (params: {
    ...
    chunks.forEach((chunk, chunkIndex) => {
        ...
        doc.text(formatIDR(order.harga_total), colX[5] - 4, grandY, { align: 'right' });
        ...
        drawKeyValue(doc, {
            ...
            value: order.payment_status === 'paid' ? 'LUNAS' : order.payment_status === 'dp' ? 'DP' : 'BELUM BAYAR',
        });
        drawKeyValue(doc, {
            labelX: left + 12,
            colonX: left + 104,
            ...
            y: paymentY + 10,
            label: 'Dibayar',
            value: formatIDR(order.dibayar ?? 0),
        });
        drawKeyValue(doc, {
            labelX: left + 12,
            colonX: left + 104,
